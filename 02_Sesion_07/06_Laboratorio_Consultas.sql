...
SELECT TransaccionID, FechaID, Cantidad, Precio_Venta FROM FactVentas;
...
SELECT TransaccionID, 
       ROUND(Precio_Venta * Cantidad * (1 - Descuento_Pct), 2) AS Venta_Neta, 
       Costo_Envio,
       (ROUND(Precio_Venta * Cantidad * (1 - Descuento_Pct), 2) - Costo_Envio) AS Margen_Aproximado
FROM FactVentas 
...
-- Éxito: 153 filas.
SELECT COUNT(*) FROM FactVentas WHERE FechaID BETWEEN 20230901 AND 20230930;
...
SELECT TransaccionID, CiudadID, Descuento_Pct 
FROM FactVentas 
WHERE CiudadID != 6 
